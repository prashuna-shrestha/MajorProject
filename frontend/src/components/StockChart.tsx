...
const StockChart: React.FC<StockChartProps> = ({ data, selectedTrends }) => {
  ...
  useEffect(() => {
    const onResize = () => setWidth(Math.max(600, Math.min(window.innerWidth - 120, 1400)));
    onResize();
  ...
  const showVolume = selectedTrends.includes('VOLUME') && chartData.some(d => d.volume !== null);
  const showRSI = selectedTrends.includes('RSI') && chartData.some(d => d.RSI14 !== null);
  ...
  return (
    ...
    >
      {/* Main price chart */}
      <Chart id={1} yExtents={(d: any) => [d.high, d.low, d.BB_UPPER, d.BB_LOWER]}>
        <XAxis />
        <YAxis />
        ...
        {/* Overlays */}
        {selectedTrends.includes('EMA12') && <LineSeries yAccessor={(d: any) => d.EMA12} stroke="#22c55e" />}
        {selectedTrends.includes('EMA26') && <LineSeries yAccessor={(d: any) => d.EMA26} stroke="#3b82f6" />}
